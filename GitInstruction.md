...
1. Делаем FORK "чужой" удаленной папки, в которую хотим внести изменения.
2. В своем удаленном репозитории копируем ссылку во вкладе Code.
3. Создаем локальную папку и делаем в ней `git clone + скопированная ссылка`
4. Создаем новую ветку и переходим в нее для внесения изменений, сохраняем.
5. Делаем `git push` - отправляем в свой удаленный репозиторий.
6. Нажимаем кнопку проверить изменения, делаем комментарий и создаем `pull request`
